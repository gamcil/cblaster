...
def subjects(hits):
    subjects = [
        ...
        classes.Subject("id3", hits[2:3], "subject2", "ipg2", 1200, 6000, "+"),
        classes.Subject("id4", hits[3:4], "subject3", "ipg3", 500, "1000", "+"),
        classes.Subject("id5", hits[4:5], "subject4", "ipg4", 6453, 8000, "+"),
        classes.Subject(name="intermediate subject", start=8100, end=9000)
    ]
    return subjects
...
def clusters(subjects):
    clusters = [
        ...
        classes.Cluster([0, 1], [subjects[1], subjects[0]], intermediate_genes=[subjects[5]], number=5)
    ]
    return clusters
...
def organisms(scaffolds):
    organisms = [
        classes.Organism("organism1", "strain_organism1",
...
def test_cluster_empty_instantiation(subjects):
    ...
    assert [[], []] == [
        getattr(classes.Cluster(subjects=[subjects[0]]), val) for val in [
            "indices",
            "intermediate_genes",
        ]
    ]
